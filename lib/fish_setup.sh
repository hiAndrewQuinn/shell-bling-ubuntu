...
setup_fish() {
  ...
  fish -c 'fish_add_path --universal --append ~/.cargo/bin' > /dev/null 2>&1 || true
  fish -c 'fish_add_path --universal --append /usr/local/go/bin' > /dev/null 2>&1 || true
  ...
  # universal vars. Belt-and-suspenders against the "uv tool update-shell"
  # footgun the test VM hit.
  mkdir -p "$HOME/.config/fish/conf.d"
  cat > "$HOME/.config/fish/conf.d/00-shell-bling-paths.fish" << 'EOF'
# Generated by shell-bling-ubuntu — see lib/fish_setup.sh
for dir in ~/.local/bin ~/.fzf/bin ~/.cargo/bin /usr/local/go/bin
    if test -d $dir
